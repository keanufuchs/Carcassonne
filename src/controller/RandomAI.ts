...
export function chooseRandomPlacement(legal: { coord: Coord; rotation: number }[], rng = Math.random) {
  if (!legal.length) return null;
  const idx = Math.floor(rng() * legal.length);
  return legal[idx];
}

export function createRandomAI(controller: GameController, aiPlayerIndex: number = 1) {
  ...
  async function performTurn() {
    ...
    try {
      ...
      if (before.phase === 'PLACING_TILE') {
        ...
        const choice = chooseRandomPlacement(legal);
        if (!choice) return;
