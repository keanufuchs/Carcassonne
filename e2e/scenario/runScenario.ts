...
import { buildBaseGameDeckIds } from '../../src/core/deck/baseGameTiles';
...
export async function runScenario(page: Page, scenario: Scenario): Promise<ScenarioRunResult> {
  ...
  const deck = scenario.deckFrom === 'base-game'
    ? buildBaseGameDeckIds()
    : [...scenario.steps.map(s => s.tile), ...(scenario.padding ?? [])];
  await page.evaluate(
...
function describeMeepleAction(step: ScenarioStep): string | undefined {
  if (step.rejectMeeple && step.meeple) return `reject segment ${step.meeple.segment}`;
  if (step.meeple) return `segment ${step.meeple.segment}`;
  if (step.skip) return 'skip';
  return undefined;
}

async function applyRotation(page: Page, rotation: number): Promise<void> {
...
async function runStep(page: Page, step: ScenarioStep, index: number): Promise<void> {
  ...
  // 2. Optional legality checks on the same pending tile before final placement.
  for (const [j, check] of (step.placementChecks ?? []).entries()) {
    const rot = check.rotation ?? 0;
    await applyRotation(page, rot);
    const legal = await page.evaluate(
      ({ coord, rotation }) => window.__carcTest!.previewPlacement(coord, rotation).legal,
      { coord: check.at, rotation: rot },
    );
    if (legal !== check.legal) {
      throw new Error(
        `${ctx}.placementChecks[${j}]: expected legal=${check.legal} at (${check.at.x},${check.at.y}) rot ${rot}, got ${legal}`,
      );
    }
  }

  // 3. Rotate to the requested placement rotation via real button clicks.
  const rotation = step.rotation ?? 0;
  await applyRotation(page, rotation);

  // 4. Place by clicking the ghost at the target coord — it must be legal there.
  ...
  // 5. Meeple step — only if the placement opened a meeple phase.
  ...
  if (phase === 'PLACING_MEEPLE') {
    if (step.rejectMeeple && step.meeple) {
      const result = await page.evaluate(
        (localId) => window.__carcTest!.tryPlaceMeepleOnLastTile(localId),
        step.meeple!.segment,
      );
      if (result.ok) {
        throw new Error(`${ctx}: meeple on segment ${step.meeple.segment} should have been rejected`);
      }
      await page.evaluate(() => window.__carcTest!.skipMeepleTurn());
    } else if (step.skip || !step.meeple) {
      await page.evaluate(() => window.__carcTest!.skipMeepleTurn());
    } else {
      await page.evaluate(
  ...
  } else if (step.meeple || step.rejectMeeple) {
    throw new Error(`${ctx}: scenario wanted a meeple action, but the placed tile produced no meeple phase.`);
  }
}
